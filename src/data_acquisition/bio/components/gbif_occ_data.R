get_occ_data <- function(species_w_codes, download_path, file_name, region = NULL, download_key = NULL, doi = NULL) {
  ...
  if (is.null(download_key) & is.null(doi)) {
    ...
    cat(yellow("If you have already made a download, press [ESCAPE], add the download key and/or doi as parameter. and run again. \n"))
    ...
    if (u_input == "") {
      ...
      out <- occ_download(
        ...
        pred("geometry", region),
        pred("format", "SIMPLE_CSV")
      )
