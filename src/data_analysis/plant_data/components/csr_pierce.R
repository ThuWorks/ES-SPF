filter_csr_list <- function(src = "./resources/synonym_checked/csr/pierce/wfo_one_checklist.csv",
  ...
  if (any(duplicated(csr_filtered[[column]]))) {
    ...
    if (any(duplicated(csr_filtered[[column]]))) {
      ...
      log_duplicates(csr_filtered, column, "data_analysis", folder, "filter_csr_priority_dups.csv")
    } else {
      cat(green("Successfully removed duplications."))
    }
  } else {
  ...
  if (!dir.exists(paste0("./outputs/data_analysis/", folder))) dir.create(paste0("./outputs/data_analysis/", folder), recursive = T)

  fwrite(csr_filtered, paste0("./outputs/data_analysis/", folder, "/sp_final.csv"), bom = T)
  
  cat("Filtering CSR values completed. \n")
