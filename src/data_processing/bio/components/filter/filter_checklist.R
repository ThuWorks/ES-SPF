filter_checklist <- function(df, column, taxon_name) {
  ...
  sp_checklist <- set_df_utf8(sp_checklist)

  if (!dir.exists(paste0("./outputs/data_procesing/", taxon_name))) dir.create(paste0("./outputs/data_procesing/", taxon_name), recursive = T)

  fwrite(sp_checklist, paste0("./outputs/data_procesing/", taxon_name, "/species_checklist.csv"), bom = T)
