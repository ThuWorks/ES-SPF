filter_blacklisted_sp <- function(sp_df, column, taxon_name, df = "./resources/synonym_checked/sp/blacklist/wfo_one_checklist.csv") {
  cat("filtering out blacklisted species \n")

  blacklist_pre <- fread(df)
  ...
  sp_blacklist <- merge(sp_df, blacklist, by = column)
  ...
  cat("Writing blacklist to: ", paste0("./outputs/data_processing/", taxon_name, "/blacklisted_species.csv"), "\n")

  fwrite(sp_blacklist, paste0("./outputs/data_processing/", taxon_name, "/blacklisted_species.csv"), bom = T)

  # Write out without the blacklisted species
  sp_rem_blacklist <- anti_join(sp_df, blacklist, by = column)
  ...
  cat("Writing without blacklisted species to: ", paste0("./outputs/data_processing/", taxon_name, "/wo_blacklisted_sp.csv"), "\n")

  fwrite(sp_rem_blacklist, paste0("./outputs/data_processing/", taxon_name, "/wo_blacklisted_sp.csv"), bom = T)
